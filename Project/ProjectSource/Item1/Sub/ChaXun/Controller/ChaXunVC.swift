...
import UIKit
import RxSwift
class ChaXunVC: DDNormalVC, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate {
    ...
    func request() {
        ...
        NetWork.manager.requestData(router: Router.get("member/\(member)/business", .api, paramete)).subscribe(onNext: { (dict) in
            ...
            self.maskLabel.isHidden = true
            GDAlertView.alert("网络错误，请稍后重试", image: nil, time: 2, complateBlock: nil)
        }, onCompleted: {
            mylog("结束")
        }) {
            mylog("回收")
    ...
    @IBAction func addressBtnAction(_ sender: UIButton) {
        ...
        let view  = AreaSelectView.init(frame: CGRect.init(x: 0, y: self.topView.max_Y, width: SCREENWIDTH, height: SCREENHEIGHT - self.topView.max_Y), title: "2223152", type: 1)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.chaxunModel = self.dataArr[indexPath.row]
        return cell
    }
