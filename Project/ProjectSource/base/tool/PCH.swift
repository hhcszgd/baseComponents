...
import Alamofire
func DDDecode<T>(_ type: T.Type, from response: DataResponse<Any>)  -> T? where T : Decodable {
    let jsonDecoder = JSONDecoder.init()
    do{
        let t = try jsonDecoder.decode(T.self, from: response.data ?? Data())
        return t
    }catch{
        mylog("jsonDecoderFailure:\(error)")
        mylog("responseData : \(response.debugDescription)")
        return nil
    }
}
