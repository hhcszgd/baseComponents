...
class ForgetPasswordVC: GDNormalVC, UITextFieldDelegate {
    ...
    override func gdAddSubViews() {
        ...
        self.resetBtn.setTitle("重置密码", for: .normal)
    ...
    @IBAction func resetAction(_ sender: UIButton) {
        ...
        let paramete = ["mobile": self.phone, "verify": self.vergion, "password": self.passwordStr, "repeat_password": self.againpasswordStr, "equipment_number": UUID, "equipment_type": "2"]
        NetWork.manager.requestData(router: Router.post("member/password", .api, paramete)).subscribe(onNext: { (dict) in
            ...
            GDAlertView.alert("网络错误", image: nil, time: 1, complateBlock: nil)
        }, onCompleted: {
            mylog("结束")
        }) {
            mylog("回收")
